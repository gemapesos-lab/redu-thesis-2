...
== Type of Research
This study adopted a *design-and-development research* approach with a *two-group baseline-intervention pilot field evaluation* #cite(<creswell-2022>). The design-and-development component focused on building the software artifact and its heuristic risk-estimation logic, while the field-evaluation component observed the system under real-world conditions.
...
*Level 3 - Pause and Reset / Short Breathing Break:* A full-screen overlay presents a brief guided breathing animation (study default: 60 seconds) when the RiskScore enters the Critical band. Following the psychology expert's recommendation, this feature is framed as a non-clinical pause-and-reset prompt rather than as a therapeutic intervention #cite(<mitsea-2023>) #cite(<yang-2023>).
...
*Session Boundary Definition:* A session begins when a target app enters the foreground (detected via `TYPE_WINDOW_STATE_CHANGED`) and ends when: (a) the target app leaves the foreground for longer than a 30-second bridge window, (b) the screen turns off, or (c) the Accessibility Service is terminated. If the user briefly switches away and returns within 30 seconds, the gap is excluded from duration and dwell-time accumulation but the session continues. Recent smartphone-logging studies that reconstruct sessions from app-event streams continue to use 45-second inter-app gaps when direct lock/unlock boundaries are unavailable #cite(<ahmed-2023>) #cite(<chen-2023>). The present system therefore treats 30 seconds as a stricter study-defined session-continuity heuristic within that recent measurement range so only very brief app switches are merged; longer away intervals are treated as new sessions. The reproducibility notes treat ±15-second perturbations as the relevant local check for this fixed study prior.
...
The −0.05 threshold follows VADER's standard classification convention, where compound scores below −0.05 are treated as negative sentiment #cite(<hutto-2014>). Caption/comment text units excluded by the language-reliability screen are removed from the text-side evaluation. No-text items routed to the VLM contribute one resolved item-level label each: `SEVERE_NEG` and `MILD_NEG` count as negative exposure, while `NEUTRAL`, `MILD_POS`, and `SEVERE_POS` count as resolved non-negative exposure. Sessions with very few resolvable units are flagged descriptively during analysis.
...
3. *Single-Expert Review and Concordance Check:* Because the review relied on a single language expert rather than a multi-rater panel, no inter-rater agreement statistic can be computed, and this is stated as a study limitation; recent sentiment-annotation work shows that valence agreement is difficult even with explicit instructions and multiple raters #cite(<krusic-2024>) #cite(<ayravainen-2025>). As a transparency safeguard, the expert's ratings were compared post hoc against the valence values deployed in the runtime lexicon: all 53 expert ratings matched the deployed values exactly (53/53 exact concordance), and all proposed stop words were confirmed as sentiment-neutral.
...
A minimum of *N = 50* participants is targeted, split as evenly as possible into an intervention group and a control group, with an initial allocation target of *n = 25* per group. This target supports a balanced pilot deployment and post-usage evaluation within an undergraduate thesis scope.
...
- *SME Evaluation:* A rating of >= 4.00 out of 5 is treated as the study-defined favorable-evaluation target for expert review; given the two-member panel, rubric ratings are reported per expert against this target.
